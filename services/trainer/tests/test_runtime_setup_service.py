...
class RuntimeSetupServiceTests(unittest.TestCase):
    ...
    def test_inspection_offers_to_remove_broken_optional_vllm(self) -> None:
        doctor = {
            "python": {"status": "ready", "version": "3.11.9", "expected": "3.11.x"},
            "gpu": {"status": "ready", "driver": {"status": "ready"}},
            "packages": [
                {
                    "name": "trl",
                    "status": "import-error",
                    "detail": "No module named 'vllm._C'",
                }
            ],
            "sandbox": {"status": "ready"},
            "environment_image": {"status": "ready"},
        ...
        self.assertEqual(result["actions"][0]["id"], "remove_broken_vllm")
        self.assertEqual(
            result["actions"][0]["command"],
            [sys.executable, "-m", "pip", "uninstall", "--yes", "vllm"],
        )
